...
class UserApi(Resource):  # 声明User资源
    ...
    def delete(self):
        id = request.args.get('id')  # id-->srt
        # user = queryById(User, id)  # 是否考虑id不存在的情况
        #
        # delete(user)
        flag = deleteById(User, id)

        return {'flag': flag, 'state': "ok",
                'msg': '删除{}成功!'.format(id)}  # 将资源添加到api对象中，并声明uri
...
class IamgeApi(Resource):
    ...
    # parser.add_argument('id', type=int,  # 参数类型
                        # required=False,  # 是否必须要的
                        # help='请提供id参数')  # 如果不存在必要参数时的提示

    # 输出定制
    @marshal_with(get_out_fields)
    def get(self):
        # self.parser.parse_args()  # 解析参数，如果参数不满足，直接返回
...
class MusicApi(Resource):
    ...
    parser.add_argument('tag', action='append', required=True, help='至少提供一个音乐标签')
    parser.add_argument('session', location='cookies', required=True, help='cookie中不存在session')

    # dest的作用，是将key替换为name，在下方的程序中使用
    # 定制输出
    music_fields = {
        'id': fields.Integer,
        'name': fields.String,
    ...
    def get(self):
        ...
        return {'msg': '查{}，找不到歌曲,标签：{}'.format(name, tags)}
...
api.add_resource(IamgeApi, '/images/')
